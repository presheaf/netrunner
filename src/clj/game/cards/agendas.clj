...
(define-card "Market Research"
  {:interactive (req true)
   ...
   :effect (effect (add-counter card :agenda 1)
                   (update-all-agenda-points)
                   (check-winner))
   :agendapoints-corp (req (if (zero? (get-counters card :agenda)) 2 4))})
...
(define-card "Oddly Specific Horoscope"
  {:constant-effects (let [cost-increaser {:req (req (= (:title target) (get-in (get-card state card) [:special :horoscope-target])))
                                           :value 3}]
                ...
                (continue-ability
                 ...
                 {:prompt "Name a Runner card"
                  :choices {:card-title (req (and (runner? target)
                                                  (not (identity? target))))}
                  :effect (effect (update! (assoc-in card [:special :horoscope-target] target))
...
(define-card "Project Ares"
  (letfn [(trash-count-str [card]
            (quantify (- (get-counters card :advancement) 2) "installed card"))]
    {:silent (req true)
     :req (req (and (> (get-counters card :advancement) 2)
     ...
     :effect (effect (show-wait-prompt :corp "Runner to trash installed cards")
               (continue-ability
                 ...
                 {:prompt (msg "Select " (trash-count-str card) " installed cards to trash")
                  :choices {:max (min (- (get-counters card :advancement) 2)
